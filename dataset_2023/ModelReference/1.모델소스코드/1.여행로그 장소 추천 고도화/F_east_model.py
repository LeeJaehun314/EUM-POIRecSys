...
Original file is located at
    https://colab.research.google.com/drive/1-9wiynJ8UzsLVskZnuEg1nYM9iszo0-x
...
visit_F = pd.read_csv('../1.inputdata/tn_visit_area_info_F.csv')

id_list=['F']

visit_data_list=[visit_F]
...
visit_final_F=visit_info

visit_final_F['ratings'] = visit_final_F[['DGSTFN', 'REVISIT_INTENTION', 'RCMDTN_INTENTION']].mean(axis=1)

visit_final_F['TRAVELER_ID'] = visit_final_F['TRAVEL_ID'].str.split('_').str[1]
...
visit_final_F['SIDO'] = visit_final_F['LOTNO_ADDR'].str.split().str[0]

dff=visit_final_F
# Group by 'FIRST_WORD' and find the most frequent 'VISIT_AREA_NM' for each group
most_frequent_visits = dff.groupby('LOTNO_ADDR')['VISIT_AREA_NM'].agg(lambda x: x.mode().iloc[0]).reset_index()

# Merge the most frequent values back to the original DataFrame based on 'FIRST_WORD'
dff = dff.merge(most_frequent_visits, on='LOTNO_ADDR', how='left', suffixes=('', '_most_frequent'))

# Update 'VISIT_AREA_NM' with the most frequent values
dff['VISIT_AREA_NM'] = dff['VISIT_AREA_NM_most_frequent'].fillna(dff['VISIT_AREA_NM'])

# Drop temporary columns used for grouping and merging
dff.drop(columns=['VISIT_AREA_NM_most_frequent'], inplace=True)

dff[['TRAVELER_ID', 'VISIT_AREA_NM','ratings','SIDO']]

"""####후처리"""

df1 = dff.rename(columns={'TRAVELER_ID': 'userID','VISIT_AREA_NM': 'itemID','ratings': 'rating'})

df1=df1[['userID','itemID','rating','SIDO']]

df1.to_csv("../2.preprocessed/dfF.csv")
...
file_path = '../4.SaveModel/model/svd_model_F.pkl'
# Save the model to the specified .pkl file
with open(file_path, 'wb') as file:
    pickle.dump(algo, file)

loaded_model1 = joblib.load('../4.SaveModel/model/svd_model_F.pkl')
...
predictions_df_F = pd.DataFrame(prediction_data)

itemID_to_SIDO = df1.set_index('itemID')['SIDO'].to_dict()
predictions_df_F['SIDO'] = predictions_df_F['itemID'].map(itemID_to_SIDO)

predictions_df_F['true_rec']=np.nan
predictions_df_F['est_rec']=np.nan

mean_true_rating = predictions_df_F['true_rating'].mean()
mean_predicted_rating = predictions_df_F['predicted_rating'].mean()
# Set 'true_rec' column based on the condition
predictions_df_F['true_rec'] = np.where(predictions_df_F['true_rating'] > mean_true_rating, 1, 0)

# Set 'est_rec' column based on the condition
predictions_df_F['est_rec'] = np.where(predictions_df_F['predicted_rating'] > mean_predicted_rating, 1, 0)

grouped = predictions_df_F.groupby(['userID', 'true_rec'])
...
for (user_id, true_rec), group in grouped:
    ...
    if true_rec == 1:
        ...
        predictions_df_F.loc[group.index, 'true_rec'] = (group['SIDO'] == majority_sido).astype(int)

grouped = predictions_df_F.groupby(['userID', 'predicted_rating'])
...
for (user_id, pred_rec), group in grouped:
    if pred_rec == 1:
        ...
        # Set 'true_rec' to 1 for the majority 'SIDO' and 0 for others
        predictions_df_F.loc[group.index, 'pred_rec'] = (group['SIDO'] == majority_sido).astype(int)

# In[13]:


print(predictions_df_F)
...
a=recall5_calculator(predictions_df_F)
print('수도권 권역의 recall@5:'+str(a))

filtered_predictions_df_F = predictions_df_F[predictions_df_F['est_rec'] == 1]

# Group by 'userID' and select top 5 items based on 'predicted_rating'
top_5_items_per_user = (
    filtered_predictions_df_F.groupby('userID')
...
df_F_final = recommendation_lists.copy()
df_F_final.to_csv('../4.SaveModel/result/testset_output/F_test_ouput.csv')
...
filtered_top_recommendations.to_csv('../4.SaveModel/result/final_output/F_top_recommendations.csv')
