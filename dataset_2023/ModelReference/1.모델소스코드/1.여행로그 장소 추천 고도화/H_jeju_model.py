...
Original file is located at
    https://colab.research.google.com/drive/18baAaobLm2l-N7nWxOIuEzPxpLc4uDMd
...
visit_H = pd.read_csv('../1.inputdata/tn_visit_area_info_H.csv')
...
dfh[['TRAVELER_ID', 'VISIT_AREA_NM','ratings','SIDO']]
...
df1.to_csv("../2.preprocessed/dfH.csv")
...
file_path = '../4.SaveModel/model/svd_model_H.pkl'
# Save the model to the specified .pkl file
with open(file_path, 'wb') as file:
    pickle.dump(algo, file)

loaded_model1 = joblib.load('../4.SaveModel/model/svd_model_H.pkl')
...
predictions_df_H = pd.DataFrame(prediction_data)

itemID_to_SIDO = df1.set_index('itemID')['SIDO'].to_dict()
predictions_df_H['SIDO'] = predictions_df_H['itemID'].map(itemID_to_SIDO)

predictions_df_H['true_rec']=np.nan
predictions_df_H['est_rec']=np.nan

mean_true_rating = predictions_df_H['true_rating'].mean()
mean_predicted_rating = predictions_df_H['predicted_rating'].mean()
# Set 'true_rec' column based on the condition
predictions_df_H['true_rec'] = np.where(predictions_df_H['true_rating'] > mean_true_rating, 1, 0)

# Set 'est_rec' column based on the condition
predictions_df_H['est_rec'] = np.where(predictions_df_H['predicted_rating'] > mean_predicted_rating, 1, 0)

grouped = predictions_df_H.groupby(['userID', 'true_rec'])
...
for (user_id, true_rec), group in grouped:
    ...
    if true_rec == 1:
        ...
        predictions_df_H.loc[group.index, 'true_rec'] = (group['SIDO'] == majority_sido).astype(int)

grouped = predictions_df_H.groupby(['userID', 'predicted_rating'])
...
for (user_id, pred_rec), group in grouped:
    if pred_rec == 1:
        ...
        # Set 'true_rec' to 1 for the majority 'SIDO' and 0 for others
        predictions_df_H.loc[group.index, 'pred_rec'] = (group['SIDO'] == majority_sido).astype(int)

print(predictions_df_H)
...
a=recall5_calculator(predictions_df_H)
print('수도권 권역의 recall@5:'+str(a))

filtered_predictions_df_H = predictions_df_H[predictions_df_H['est_rec'] == 1]

# Group by 'userID' and select top 5 items based on 'predicted_rating'
top_5_items_per_user = (
    filtered_predictions_df_H.groupby('userID')
...
df_H_final = recommendation_lists.copy()
df_H_final.to_csv('../4.SaveModel/result/testset_output/H_test_ouput.csv')
...
filtered_top_recommendations.to_csv('../4.SaveModel/result/final_output/H_top_recommendations.csv')
